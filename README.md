...
%The output of the Lyapunov critic network is a square term, which is always non-negative. More specifically, we use a fully-connected MLP with two hidden layers and one output layer with different units as in the above table, outputting the feature vector <img src="http://latex.codecogs.com/gif.latex?f_\phi(s,a)"/>. The Lyapunov value is obtained by <img src="http://latex.codecogs.com/gif.latex?L_c(s,a)=f_\phi^T(s,a)f_\phi(s,a)"/>. All the hidden layers use Relu activation function and we adopt the same invertible squashing function technique as~\citet{haarnoja2018soft} to the output layer of the policy network.
...
%<div align=center><img src = "https://github.com/hithmh/Actor-critic-with-stability-guarantee/blob/master/figures/parametric.jpg" width=800 alt="figure"></div>
